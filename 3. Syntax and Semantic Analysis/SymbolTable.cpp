...
public:
    ...
    SymbolInfo* LookupCurrent(SymbolInfo symbol){
        return currentScope->Lookup(symbol);
    }

    void PrintCurrentScopeTable(FILE* out){//taken the output file pointer
        fprintf(out,"\tScope Table#%d\n",currentScope->ID);
        currentScope->Print(out);
    }
    void PrintAllScopeTable(FILE* out){//taken the output file pointer
